...
public class m_notice{
	...
	String subject,writer,ncheck,texts;
	
	//즉시 실행 메소드에서 첨부파일을 저장하는 역할만 합니다.
	public m_notice(Part nfile, String subject, String writer, String ncheck,String texts, HttpServletRequest request)throws Exception {
	...
	private String fileok(String data) throws Exception{
		if(data.equals("error")) {
			...
			try {
				...
				this.sql = "insert into notice (nidx, ncheck,subject,writer,texts,filenm,nfile,ndate)"
						+ "values ('0',?,?,?,?,?,?,now())";
